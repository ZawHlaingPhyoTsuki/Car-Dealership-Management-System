...
export const columns: ColumnDef<Car>[] = [
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className="min-w-0">
                    ...
                    {car.licenseNumber && (
                        <div className="text-sm text-gray-500 truncate">{car.licenseNumber}</div>
                    )}
                </div>
            );
        },
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const { companyProfit, companyPercentage } = companyProfitAndPercentageCalculator(
                price,
                percentage
            );
            return (
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const { shareholderProfit, shareholderPercentage } =
                shareholderProfitAndPercentageCalculator(price ?? 0, percentage);
            return (
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const investmentAmount = Number.parseFloat(row.getValue('investmentAmount'));
    ...
    {
        accessorKey: 'soldAt',
        header: ({ column }) => <DataTableColumnHeader column={column} title="Sold Date" />,
        cell: ({ row }) => {
            const car = row.original;
...
function CarActions({ car }: { car: Car }) {
    ...
    return (
        <>
            ...
            <EditCarSharerDialog car={car} open={showEditDialog} onOpenChange={setShowEditDialog} />
